 ...
 ******************************************************************************
  ...
  函数列表   :
              ...
              hi_Hex_2_Asc
              hi_Hex_2_Dec
              hi_md5_auth_build_resonse
              hi_md5_auth_cvtHex
              hi_md5_auth_digest_CalcHA1
              hi_md5_auth_digest_CalcResponse
              hi_md5_encrypt
              hi_url_decode
              hi_url_encode
              MD5Transform
...
***********************************************************/
char * hi_url_encode(char const *s, int len, unsigned char *to, int *new_length) 
{ 
...
void hi_md5_auth_cvtHex( HASH bin, HASHHEX hex )
{
    unsigned short i;
    unsigned char j;
...
void hi_md5_auth_digest_CalcHA1(const char* authId, const char* password,
                             ...
                             const char* cnonce, HASHHEX sessionKey )
{
    ...
    hi_md5_auth_cvtHex( ha1, sessionKey );
}

void hi_md5_auth_digest_CalcResponse( HASHHEX ha1, const char* nonce, const char* nc,
                                  ...
                                  const char* uri, HASHHEX entity, HASHHEX response )
{
    ...
    hi_md5_auth_cvtHex( respHash, response );
}
                                 ...
                                 const char* method,const char*  uri)
{
    ...
    hi_md5_auth_digest_CalcHA1(auth, pass, alg, realm, nonce, cnonce, ha1 );
    hi_md5_auth_digest_CalcResponse(ha1, nonce, nc, cnonce, qop, method, uri, ha2, response );
 ...
 * DES key schedule
 */
void hi_des_set_key( des_context *ctx, uchar key[8] )
{
    int i;
 ...
 */
void hi_des_encrypt(uchar *key, uchar input[8], uchar output[8])
{
	des_context ctx;
	hi_des_set_key(&ctx,  key);
 ...
 */
void hi_des_decrypt(uchar *key, uchar input[8], uchar output[8] )
{
	des_context ctx;
	hi_des_set_key(&ctx,  key);
 ...
 */
void hi_des3_set_2keys( des3_context *ctx, uchar key[16] )
{
    int i;
 ...
 */
void hi_des3_set_3keys( des3_context *ctx, uchar key[24] )
{
    int i;
...
int	hi_Hex_2_Asc(unsigned char *pSrc,int len,unsigned char *pDest)
{
	...
	for(i = 0; i < len/2; i++, pSrc += 2)  
	{
		...
		if(hi_Hex_2_Dec(*pSrc) < 0 || hi_Hex_2_Dec(*(pSrc+1)) < 0)
			return -1;
		*(pDest+i) = (hi_Hex_2_Dec(*pSrc) << 4 &0xf0) | (hi_Hex_2_Dec(*(pSrc+1)) & 0xf);
	}
...
int hi_Asc_2_Hex(unsigned char *pSrc,int len,unsigned char *pDest)
{
	...
	{
		*pDest = hi_Dec_2_Hex((*pSrc & 0xf0) >> 4); //high four bits
		 pDest++;
		*pDest = hi_Dec_2_Hex(*pSrc & 0x0f);        //lwo four bits
		 pDest++;
	}
	return (len*2);
}
