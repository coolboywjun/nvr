 ...
 ******************************************************************************
  文 件 名   : hi_rtsp_h264.c
  版 本 号   : v2.0
  作    者   : 梁锦明
  生成日期   : 2011年9月8日
  最近修改   :
  功能描述   : rtsp 发送h264视频实现
  函数列表   :
              rtsp_h264_find_nal
              rtsp_h264_get_media_info
              rtsp_h264_send_nal
              rtsp_h264_video_send
  修改历史   :
  1.日    期   : 2011年9月8日
    作    者   : 梁锦明
    修改内容   : 创建文件
...
extern int hi_find_nal_unit(char *pData, int nDataLen);
...
*/	
	while(nCurPos < nFrameLen && nFrameLen > 0)
	{
										...
										nFrameLen - nCurPos)) < 0)
		{
			list_for_each_entry(pSess, pSessHead, transList)
			{
				rtsp_h264_send_nal(pSess, &szBuff[nCurPos], nFrameLen - nCurPos,
						1, nPts);
			}
			break;
		}
		...
		if(nNalUnitType != 0/*nal_unit_type == 1 || nal_unit_type == 5*/)
		{
			list_for_each_entry(pSess, pSessHead, transList)
			{
				rtsp_h264_send_nal(pSess, &szBuff[nCurPos], nNalLen, 0, nPts);
			}
		}
