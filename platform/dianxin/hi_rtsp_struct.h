...
#define RTP_MAX_PACKET_BUFF 	 1024
...
typedef struct _RTP_SENDER_S
{
	...
	int					nRtpNatPort[RTSP_MAX_STREAM_TYPE];
	int					nRtcpNatPort[RTSP_MAX_STREAM_TYPE];
    ...
    unsigned long			u32FirstSndPts[2];
    unsigned long			u32FirstSndTs[2];
}RTP_SENDER_S;
...
{           
	...
	char 			szBuffData[RTSP_MAX_PROTOCOL_BUFFER];
	int					nDataSize;
	...
	int 				isHistoryStream;		/*是否实时流, add by xq*/
	char				szVodFileName[32];
	char				szBeginTime[32];
	time_t			beginTime;
	time_t			endTime;
	int				totalSec;			/* 录像长度*/
	int				scale;			/* 以1000为单位，大于1000加速，小于1000慢速*/
	int				seekSec;
	int				isInner;			/* 是否内部转发*/
	
	void 			*vodHandle;

	char				turnServerIp[64];
	int				turnServerPort;
	char				remoteIp[64];
	int				remotePort;
	int				isTurnAuth;
