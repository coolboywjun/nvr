...
#define DHCP_INITIAL_RTO	(1*1000000)
#define DHCP_MAX_RTO		(2*1000000)
...
typedef struct dhcpEtherInfo* dhcpEtherInfoPtr;

typedef struct dhcpClientInfo
{
  char IfName[32];
  ...
  dhcpEtherInfoPtr etherInfo;
  struct dhcpClientInfo *next;
}dhcpClientInfo;

typedef struct dhcpEtherInfo
{	
  char IfName[32];
...
typedef void (*buildUdpIpMsg)(udpipMessage *pDhcpMessage, uint8_t *ClientHwAddr, 
				uint32_t xid, dhcpInterface *DhcpIface, dhcpOptions *DhcpOptions);
...
int dhcpBound(dhcpClientInfo *clientInfo);
