...
int arpCheck(int dhcpSocket, const char* IfName, unsigned char *ClientHwAddr, dhcpInterface *DhcpIface)
{
    ...
    do
    {
        ...
        memcpy(addr.sa_data,IfName,strlen(IfName)+1);
        if ( TokenRingIf )
    ...
    do
    {
        ...
        if ( recvfrom(dhcpSocket,&ArpMsgRecv,sizeof(arpMessage),0,
                (struct sockaddr *)&addr,(socklen_t *)&j) == -1 )
        {
    ...
    while ( peekfd(dhcpSocket,50000) == 0 && trynum++ < 10);
    //}
    //while ( 1 );
    return 0;
}
//#endif
/*****************************************************************************/
int arpRelease(int dhcpSocket, const char* IfName, unsigned char *ClientHwAddr, dhcpInterface *DhcpIface)  /* sends UNARP message, cf. RFC1868 */
{
...
/*****************************************************************************/
int arpInform(int dhcpSocket, const char* IfName, unsigned char *ClientHwAddr, dhcpInterface *DhcpIface)
{
    arpMessage ArpMsgSend;
    struct sockaddr addr;
    int len;
