...
LIBSPEC struct UPNPDev * upnpDiscover(int delay, const char * multicastif,
              const char * minissdpdsock, int sameport, const char *hostIp)
{
	struct UPNPDev * tmp;
...
#if 0 // !defined(WIN32) && !defined(__amigaos__) && !defined(__amigaos4__)
 ...
 * in order to give this ip to setsockopt(sudp, IPPROTO_IP, IP_MULTICAST_IF) */
	...
	for(n = 0;;)
	{
...
#if DEBUG		
			printf("upnpDiscover: Sending %s", bufr);
#endif
		...
		if (n < 0) {
			...
			int stsize=0;
#ifdef DEBUG		
	        printf("upnpDiscover: Recving %d byte(s) :\n%s\n", n, bufr);/* affichage du message */
#endif        
			...
			if(st&&descURL)
			{
				...
				if(tmp || (hostIp != NULL && strstr(descURL, hostIp) == NULL))
					continue;
