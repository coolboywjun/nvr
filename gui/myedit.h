...
/*add by luoping 2010.9.16  for edit foucs*/
#define EST_FOCUS     0x00000010L
...
typedef struct tagEDITDATA
{
    ...
    RECT    rcVis;          // content rect  文字区域
    int     nVisW;          // visible area width//可见区域的宽度
    ...
    gal_pixel color_ft;           // front color
    gal_pixel color_ft_hilight;
    gal_pixel color_boder;         //boder color
    gal_pixel color_boder_hilight;

}EDITDATA;
typedef EDITDATA* PEDITDATA;
...
//只能输入数字    在附加数据里面可以设置一个上线
#define ES_INPUT_NUM           0x00004000L
