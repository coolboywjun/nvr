...
int svGetItemRect (PSVDATA psvdata, HSVITEM hsvi, RECT *rcItem, BOOL bConv)
{
...
/* -------------------------------------------------------------------------- */
static void scrollview_draw1 (HWND hWnd, HDC hdc, PSVDATA psvdata)
{
    list_t *me;
    PSVITEMDATA pci;
    RECT rcDraw;
	...
	{
            if (rcDraw.top < rcVis.top) {
		h = pci->nItemHeight;
		continue;
	    }
	    if (rcDraw.bottom > rcVis.bottom)
		break;
	}
        ...
        if (!(GetWindowStyle(hWnd) & WS_VSCROLL))
        {
	    	...
	    	pscrdata->vScrollPageVal += h;
    }
}

#if 0
/* adjust the position and size of the svlist window */
static void scrollview_set_svlist (HWND hWnd, PSCRDATA pscrdata, BOOL visChanged)
{
    PSVDATA psvdata = (PSVDATA) GetWindowAdditionalData2 (hWnd);
...
int svlist_init (HWND hWnd, PSVLIST psvlist)
{
 ...
 */
int svInitData (HWND hWnd, PSVDATA psvdata)
{
    RECT rcWnd;
...
/* --------------------------------------------------------------------------------- */
int MyScrollViewCtrlProc (HWND hWnd, int message, WPARAM wParam, LPARAM lParam)
{
...
//FIXME, moved to listmode.c
    case MSG_PAINT:
    {
        ...
        scrollview_draw1 (hWnd, hdc, psvdata);
        EndPaint (hWnd, hdc);
        return 0;
    }
...
BOOL RegisterMyScrollView (void)
{
    WNDCLASS WndClass;

    WndClass.spClassName = "scrollview_ex";
    WndClass.dwStyle     = WS_NONE;
    WndClass.dwExStyle   = WS_EX_NONE;
    WndClass.hCursor     = GetSystemCursor (IDC_ARROW);
    WndClass.iBkColor    = GetWindowElementColor (BKC_CONTROL_DEF);
    WndClass.WinProc     = MyScrollViewCtrlProc;
