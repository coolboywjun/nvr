...
void __fastcall TUDPThread::UpdateFrom()
{
    ...
    Form1->btnUpdateFile->Enabled = false;
    Form1->btnCfg->Enabled = false;
    Form1->btnUpdate->Enabled = false;
    Form1->ComboBox1->Enabled = false;
}
void __fastcall TUDPThread::UpdateCfgLable()
{
     Form1->lblConfig->Caption = "成功配置平台类型: " + IntToStr(this->cfgNum);
}
//---------------------------------------------------------------------------
void __fastcall TUDPThread::Execute()
{
    ...
    while(!this->Terminated)
    {
        ...
        if(pHead->uMagic != 33080)
            continue;
        if(pHead->uType == PSTJ_BROADCASE_SCAN)
        {
        ...
        else if(pHead->uType == PSTJ_SET_PLATFORM_TYPE)
        {
            if(pHead->ret == 0)
            {           
                this->cfgNum++;
                Synchronize(UpdateCfgLable);
                //Form1->lblConfig->Caption = "成功配置平台类型: " + IntToStr(this->cfgNum);
            }
        }
    }
...
int __fastcall TUDPThread::SendBroadPlatType(unsigned short platType)
{    
    char buffer[1024];
    PSTJMsgHeader pHead;
    SOCKADDR_IN udpAddr;
    ...
    pHead = (PSTJMsgHeader)buffer;
    pHead->uMagic = HEAD_MAGIC;
    pHead->uType = PSTJ_SET_PLATFORM_TYPE;
    pHead->ret = 0;
    memcpy(pHead->dat, &platType, sizeof(platType));
...
int __fastcall TUDPThread::GetCfgNum()
{
    //TODO: Add your source code here
    return this->cfgNum;
}

void __fastcall TUDPThread::ResetCfgNum()
{
    //TODO: Add your source code here
    this->cfgNum = 0;
}
