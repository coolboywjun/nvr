...
RTSP_TKN_S RTSP_Status [] =
{
	...
	{"Method Not Valid In Tnvrs State", 455},
...
// Tnvrs routine is from OMS.
{
	...
	while(ml <= pSess->nMsgSize)
	{
		...
		*/
		...
		while(!eomh && ((ml + tc + ws) < pSess->nMsgSize))
		{
			...
			if(c == '\r' || c == '\n')
				...
				ws++;	/* wnvrte space between lf & cr is sloppy, but tolerated. */
			else
				break;
		}
		/*
		...
		* section of the message.  Tnvrs is in keeping with RFC 2068,
